...
For thesis-writing changes, use this workflow: revise the English source first, align the Chinese version next, compile the relevant PDF, then commit and push the source and generated PDF together. If LaTeX reports errors but still produces a PDF, commit and push that PDF only when the user explicitly asks for that specific run; otherwise preserve the last good PDF.
