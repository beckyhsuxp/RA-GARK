"""KGRec baseline training script.

Trains the KGRec model with:
  - BPR loss on the original (full) graph view
  - InfoNCE contrastive loss between original and rationale-dropped views

Run:
    python -m baselines.train_kgrec
"""
...
from baselines.kgrec import KGRec
...
CL_WEIGHT = 0.01      # InfoNCE weight
DROP_PROB = 0.3       # max edge-drop probability for rationale dropout


def train_kgrec(cfg: Config, device: torch.device) -> None:
    set_seed(cfg.seed)
    ...
    log.info("KGRec: cl_weight=%.3f  drop_prob=%.2f", CL_WEIGHT, DROP_PROB)
    ...
    for epoch in range(cfg.epochs):
        ...
        for users, pos_items, neg_items, _ in loader:
            ...
            emb_full, emb_drop = model.propagate_two_views()

            # ---- BPR on the original graph ------------------------------
            ...
            # ---- Cross-view CL: original ↔ rationale-dropped ------------
            u_drop = emb_drop[users]
            pos_drop = emb_drop[n_users + pos_items]
            loss_cl = (
                infonce_loss(u_full, u_drop, cfg.temp)
                + infonce_loss(pos_full, pos_drop, cfg.temp)
            )
...
if __name__ == "__main__":
    ...
    cfg = Config()
    cfg.epochs = 80
    cfg.model_save_path = "best_model_kgrec.pth"

    train_kgrec(cfg, _device)
