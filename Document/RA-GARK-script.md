...
我們的工作可以理解成延續了 KGRec 的核心思路——KG 訊號需要被主動挑選，而不是全部注入——但在四個地方做了調整。

第一，理由挑選的粒度從邊的層級提升到 item 的 aspect slot，我們每個 item 維護四個 aspect slot，讓挑選結果可以直接對應「這個 item 最突出的 aspect 是什麼」，比較容易解讀。

第二，機制從離散的隨機丟棄改成可微分的 softmax attention，推論的時候可以直接看每個 user–item pair 對應的 aspect 權重。

第三，歸一化方式從 sigmoid 改成帶溫度縮放的 softmax，這個改動帶來了 7% 的 NDCG 差距，是全文消融裡單項影響最大的。

第四，架構從 KGAT 聚合器改成 LightGCN side channel，在稀疏 knowledge graph 下 collaborative signal 更純淨。
...
這邊我想多說一下閘門和 attention 這兩種整合機制的區別，因為我們的架構裡其實兩者都用了，但用在不同的地方。

attention 機制，像 KGAT、KGRec、DIN 這些方法用的，是一個分佈式的權重——所有候選的權重加總等於一，語意是「這些候選互相競爭，誰比較重要」。它的優點是表達能力強，適合在多個同類候選之間做相對比較。

閘門機制，像 LSTM 的遺忘閘、或者混合專家模型裡的路由閘門，輸出的是一個介於零和一之間的標量，語意是「這條路徑要不要開、開多大」，是一個開關的概念。它的優點是訓練穩定，而且天然適合「是否採用某條管線」這種決策。

我們的架構裡，aspect 選擇那一層用的是 attention——四個 aspect slot 是同類候選，用 softmax 讓它們互相競爭來決定誰代表這個 item。視角融合那一層用的是閘門——本地和全域是本質異質的兩條管線，我們需要的是「要不要引入 knowledge graph 這條通道」這個開關決策，而不是讓兩者競爭誰比較重要。

而且閘門機制跟我們的 local-biased 初始化設計天然相容——把偏置初始化為正五，閘門就從幾乎關閉的狀態起步，提供了結構上的安全退化保證。
...
在理由感知設計裡，attention normalization 方式的差距達到了 7 個百分點的 NDCG，遠超一般調參能帶來的差距。

這張表整理了兩種方式的比較。Sigmoid 的語意假設是各元素獨立評估重要性，訓練行為是趨向飽和均勻；softmax 的語意假設是少數選項互相競爭勝出，訓練行為是產生具鑑別力的加權。

我們想強調的是，這個觀察不是在說 softmax 一定比 sigmoid 好。DIN 用 sigmoid 對歷史行為做 attention 是完全合理的，因為那個場景的語意是各個歷史行為獨立評估跟目標的相關性。問題在於，在 aspect 選擇這個場景，語意是「少數幾個 aspect 主導了這個 item 被推薦的原因」，在這個語意下 softmax 更符合需求。
