...
When the KG is unreliable, what the architecture needs is not a better KG integrator but a switch that can turn the KG off when needed.
...
測試更密集的 KG benchmark，並分析 user-level rationale differences 何時會更明顯。
