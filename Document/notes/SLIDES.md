...
**LightGCN** *(He et al., 2020)* — 純協同過濾的強基準

- 移除 GCN 非線性激活與特徵轉換，僅保留線性鄰域聚合
- 本文 **local view 直接沿用（K=2）**，不疊加任何 KG 操作以保持協同訊號純淨
- 純 LightGCN 在本文設定即達 NDCG@20 = 0.1179，是設計上**「不能更差」的硬性底線**

**KGAT** *(Wang et al., 2019)* — KG 深度融合的代表範式

- 合併 user–item 圖與 KG 為 CKG，bi-interaction aggregator 將 KG entity 嵌入**直接帶入 user/item 表示**
- KG 豐富時有效；**稀疏 KG 下雜訊一併被融入** → 本文設定：0.1079 < LightGCN
- 本文反向選擇：**KG 走獨立側通道**，不污染協同訊號的梯度路徑
...
> **本文兩視角結構上異質**：local 為 user–item 圖的協同訊號，global 為 KG 語意訊號 — 不是 SGL / DCCF 那種同圖擾動視角，需要**顯式 gate 控制混合比例**而非僅靠 CL 隱式對齊。
...
**文獻背景**：DIN [Zhou et al., 2018] 用 sigmoid，NAIS [He et al., 2018] / AFM [Xiao et al., 2017] 用 softmax — 既有 attention 模組對歸一化方式無共識，多依語意假設選擇。
