# Product Recommendation via Rationale-Aware Gating over Sparse Review-Aspect Knowledge Graphs (RA-GARK)
...
- [2. Related Work](#2-related-work)
  ...
  - [2.4 Gating vs Attention for Multi-Signal Fusion](#24-gating-vs-attention-for-multi-signal-fusion)
  - [2.5 Attention Normalization 的方法論觀察](#25-attention-normalization-的方法論觀察)
  - [2.6 小結](#26-小結)
...
## 2.4 Gating vs Attention for Multi-Signal Fusion

在推薦系統中整合多個訊號源（多視角、多模態、多圖），既有文獻中主要有兩種機制：**attention** 與 **gating**。本文對此兩者皆有採用，但作用於不同層級；由於兩者經常被視為可替換的選擇，我們在此節明確區分其適用情境。

**Attention-based fusion**
多數 KG-aware 方法（KGAT、KGRec 等）在 GNN 傳遞中以 attention 為核心機制：對鄰居、邊或候選視角計算權重分佈，並以加權平均進行整合。Attention 的優勢是**富表達力**（對每對 source–target 產生不同權重）與**跨元素競爭**（softmax 的歸一化強制 $\sum_a w_a = 1$），適合「**多個候選互相比較**」的場景——例如從多個 aspect 中挑選最能代表 item 的面向、或從多個歷史 item 中挑選與當前目標最相關者 [Zhou et al., 2018]。

**Gating-based fusion**
LSTM/GRU、Mixture-of-Experts (MoE) [Shazeer et al., 2017]、Highway Networks [Srivastava et al., 2015] 等架構廣泛採用 gating：對每條訊號流學習一個標量（或向量）閘 $\alpha \in (0, 1)$，以 $\alpha \cdot A + (1-\alpha) \cdot B$ 形式組合。Gating 的優勢是**路徑可獨立開關**（當某條訊號流無訊號時 α 可學到極值、對應路徑自動關閉）與**訓練穩定性**（單一標量的優化動力學比 attention 分佈更單純）。

**本文的取捨**
RA-GARK 在兩個不同的層級同時採用兩種機制：

- **Aspect 選擇層級**（§3.4.3）：採用 **attention**。A = 4 個 aspect 槽是「**同類候選**」，模型需決定「哪個面向最能代表此商品」——此為典型的「多候選比較」情境，softmax-based attention 的跨元素歸一化最為合適。
- **視角融合層級**（§3.5）：採用 **gating**。Local view（協同訊號）與 global view（KG 語意訊號）是**本質異質的兩條管線**，並非「同類候選互相比較」——此時需要的是「**此訊號是否該進入 scoring**」的**開關機制**，而非「此訊號相對另一個多重要」的**分配機制**。此外，gating 的標量閘門還能與「本地偏置初始化」（§3.5.2）自然結合，提供結構上的**可安全退化保證**：若 global view 無法提供有用訊號，閘門保持關閉，模型退化為接近純 LightGCN 的形式。這對我們關注的稀疏 KG 情境尤為重要。

換言之，我們將兩種機制區分為：

> **Attention 用於「選擇」（competition）；gating 用於「開關」（admission）。**

此區分在既有多視角推薦與 KG-aware 推薦文獻中較少被明確討論；多數方法會將兩條異質管線以 attention 加權融合，這在我們的稀疏 KG 實驗中並不穩健。我們認為「選擇 vs 開關」的對比對日後 KG-aware 設計具實務參考價值。

## 2.5 Attention Normalization 的方法論觀察
...
![Figure 1. RA-GARK 整體架構（橫版）。兩條平行的 swim lane 分別是 Local View（LightGCN，協同訊號）與 Global View（KG-aspect 語意訊號，透過 Rationale Masking 取出 i_glo）；兩視角各自送入 user / item fusion gate 合併，產出 u_final 與 i_final 的內積作為分數。L_aCL 與 L_uCL 的虛線表示 stop-grad 的跨視角對比學習路徑。本文的三項核心設計分別體現於：(i) item_kg_aspects 採用 KG-SVD 初始化（§3.4.2），(ii) Rationale Masking 採 softmax + τ = 0.5 形式（§3.4.3），以及 (iii) fusion gate 的 bias 初始化為 +5（§3.5.2）。](figures/architecture.png)

RA-GARK 由四個模組組成（Figure 1）：
...
> **產生 Figure 1 的方式**：執行 `python figures/architecture.py`，輸出 `figures/architecture.png` 與 `figures/architecture.pdf`。上述圖檔需要先產生後，Markdown preview / Pandoc / LaTeX 匯出才能顯示圖像；若使用 GitHub/VSCode 預覽而圖檔尚未存在，該圖區塊會顯示為 broken image。
