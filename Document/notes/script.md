...
我們只在 user-item bipartite graph 上做傳播，而且只用訓練互動資料。`A_norm` 是 normalized adjacency matrix，也就是把每個節點的鄰居資訊做歸一化後再聚合；`E^(l)` 是第 `l` 層的 embedding。這個過程沒有 KG edges，也沒有額外的 nonlinear transformation。
