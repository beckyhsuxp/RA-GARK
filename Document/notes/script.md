...
對應這兩個問題，我們的答案是：KG 不應該是評分流程裡必須使用的組件，而應該是一條可以被 gate 控制的側通道。後面的模型設計都會圍繞這個原則展開。
...
接著是 contrastive KG learning 和 gating。簡單說，KGCL 和 MCCLK 會用對比學習強化 KG 表示，gating 方法則是學習不同訊號要怎麼分配權重。
