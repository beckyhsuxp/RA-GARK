...
模型最後的 score 是 user 和 item 的 final representation 做內積。BPR 是用正負樣本做排序學習的 loss。對每個已觀察互動，我們再抽一個沒互動過的 item 當負樣本，目標是讓真正互動過的 item 分數高於未互動 item。BPR 負責把排序學好，gate 則是先把 local 和 global 的融合控制住。
