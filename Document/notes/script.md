...
先從 IDF-weighted matrix 做 truncated SVD，也就是只保留前 k 個成分，這裡 k 等於 A 乘 d。你可以把這一步想成把加權後的矩陣拆成三個部分：左邊的 `U sub k`、中間的 `Sigma sub k`、以及右邊的 `V sub k transpose`。其中 `U sub k` 和 `Sigma sub k` 會組成 `E sub KG`，也就是每個 item 的初始 KG 表示。
