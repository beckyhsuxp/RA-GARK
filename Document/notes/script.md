...
這一頁看訓練設定。表格裡挑幾個重要的就好，像是 embedding dimension、LightGCN layers、aspect slots、rationale temperature、fusion-gate bias、contrastive weight、optimizer、learning rate 和 batch size。這個 benchmark 本身就很稀疏，所以我們才特別在這種 training setup 下測試方法。
