...
## Slide 7 — Related Work: KGCL & MCCLK
...
## Slide 9 — Related Work: Rationale Paradigm — Shared vs Diverged
...
## Slide 10 — Related Work: Dual-View Recommenders
...
## Slide 11 — Related Work: Gating for Heterogeneous View Fusion
...
## Slide 12 — Related Work: Gating Gap in KG-aware Recommendation
...
## Slide 13 — Related Work: Attention Normalization
...
## Slide 18 — Global View: KG-SVD Initialization
...
## Slide 19 — Global View: Softmax Aspect-Saliency Attention
...
關鍵設計是用 softmax 而不是 sigmoid。如果用 sigmoid，各個 aspect 的權重是獨立算的，訓練過程中傾向飽和到接近一，等於沒有做選擇，退化成取平均。改成 softmax，因為所有權重加總必須等於一，各個 aspect 之間互相競爭，才能產生有鑑別力的加權。這個差距我們在 Slide 13 已經講過，在我們的資料集上達到 7 個百分點，是全文消融裡單項影響最大的。
...
## Slide 20 — Local-Biased Fusion Gate
...
## Slide 21 — Cross-View Contrastive Regularization
...
## Slide 23 — Dataset & KG Construction
...
第一個，把 softmax aspect attention 改回 sigmoid，NDCG 掉到 0.1151，下降了 7%。這幾乎等同於說「用了 knowledge graph 反而變差了」，因為 0.1151 比純 LightGCN 的 0.1179 還低。這個結果印證了我們在 Slide 13 講的：在 aspect 選擇的語意下，歸一化方式的選擇至關重要。
...
## Slide 26 — Methodological Insight

除了數字上的結果，我們還想再把 Slide 13 的那個觀察更完整地說一遍，但我會在這邊很小心地措辭，避免過度索取。
