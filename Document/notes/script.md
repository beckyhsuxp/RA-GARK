...
這張圖從左往右看，先把 `u_loc` 和 `u_glo` 串起來，得到 `Gate_u([u_loc || u_glo])` 這個輸入。這裡先以 user-side 為例。更完整地說，`Gate(z)` 是 `sigma(w^T tanh(W z) + b)`，所以 `alpha_u` 就是把 user-side 的兩個表示送進這個 gate 後得到的輸出。
