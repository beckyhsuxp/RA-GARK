...
這張圖從左往右看，先把 `u_loc` 和 `u_glo` 串起來，得到 `Gate_u([u_loc || u_glo])` 這個輸入。這裡先以 user-side 為例。更完整地說，`Gate(z)` 就是那個帶 `tanh` 的小型 MLP，再接上一個 sigmoid head；其中 `w^T tanh(W z) + b` 可以理解成 MLP 的最後一層，前面先做隱層變換，最後再加上 bias `b`。所以 `alpha_u` 就是把 user-side 的兩個表示送進這個 gate 後得到的輸出。
