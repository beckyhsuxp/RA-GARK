...
當 KG 不可靠時，架構最需要的不是更強的 KG 整合器，而是一個可以在需要時把 KG 關掉的開關。這篇工作的主要貢獻有四個：gateable KG side channel、KG-SVD initialization、softmax rationale masking、local-biased fusion gate。
