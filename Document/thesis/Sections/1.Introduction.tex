...
\begin{enumerate}
  ...
  \item \textbf{RA-GARK, a dual-view architecture} that integrates collaborative and KG-derived signals through a side-channel design rather than deep fusion. RA-GARK consists of three core ingredients, each of which is independently necessary:
    \begin{itemize}
      \item \textbf{Softmax aspect-saliency attention} (Section~\ref{sec:softmax}). Per-item KG semantics are stored across multiple aspect slots; a softmax-normalized attention selects which aspect best represents the item for a given user--item pair. Removing softmax in favor of sigmoid degrades NDCG@20 by $7.0\%$, the largest single-component effect we observe.
