...
\begin{enumerate}
  ...
  \item \textbf{RA-GARK，一個雙視角架構}：以旁路通道而非深度融合的方式整合協同訊號與 KG 衍生訊號。RA-GARK 由三項核心要素組成，且彼此皆獨立必要：
    \begin{itemize}
      \item \textbf{Softmax aspect-saliency 注意力}（第~\ref{sec:softmax}~節）。每件物品的 KG 語意分散儲存於多個 aspect 槽位中；softmax 正規化的注意力為給定的使用者—物品配對選取最足以代表該物品的 aspect。將 softmax 改為 sigmoid 會使 NDCG@20 下降 $7.0\%$，是我們所觀察到單一元件影響最大者。
