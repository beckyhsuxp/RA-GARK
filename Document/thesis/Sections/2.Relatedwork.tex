...
\begin{itemize}
  \item \textbf{Rationale granularity.} KGRec selects at the edge level (KG triples). RA-GARK aggregates each item's KG semantics into $A=4$ aspect slots and selects at the slot level, which is coarser but yields directly interpretable per-item aspect preferences.
