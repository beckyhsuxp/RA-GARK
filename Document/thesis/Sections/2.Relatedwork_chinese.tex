...
本章從五個面向回顧先前研究，這些面向共同促成 RA-GARK 的設計。第~\ref{sec:rw-foundations-zh}~節介紹協同與 KG-aware 推薦基礎，包含 LightGCN、KGAT、對比式 KG 方法與 KGRec。第~\ref{sec:rw-paradigm-zh}~節以「KG 可信度」為視角，對比我們與 KGRec 的合理化典範。第~\ref{sec:rw-gating-zh}~節回顧閘控機制並指出 KG-aware 推薦中缺少融合閘設計。第~\ref{sec:rw-attention-norm-zh}~節討論注意力正規化，第~\ref{sec:rw-summary-zh}~節總結 RA-GARK 相對於上述工作的定位。

\section{基礎與 KG-Aware 基線}
