...
在 RA-GARK 中，我們\emph{原封不動}採用 LightGCN（$K=2$）作為區域視角：此分支內不施加任何 KG 相關運算。動機有二。其一，在我們的稀疏評論型 KG 上，純 LightGCN 已達 NDCG@20 = $0.1179$，超越所有我們所評估的 KG-aware 基線。任何偏離此強預設的設計，皆須可被證實能改善而非劣化結果，這是本論文反覆訴諸的硬性設計約束。其二，將 KG 訊號隔離至獨立的全域視角（第~\ref{ch:methodology-zh}~章），可避免 KG 雜訊污染協同表示的梯度流。
...
\begin{table}[ht]
\centering
\caption{合理化典範對比：KGRec 與 RA-GARK 於四項設計軸線。}
...
\begin{table}[ht]
\centering
\caption[代表性 KG-aware 推薦器所採用的整合機制。]{代表性 KG-aware 推薦器所採用的整合機制。皆未提供在不可靠 KG 下的架構優雅退化能力。}
...
我們刻意作出一個範圍狹義的主張。我們不主張 RA-GARK 是 KG-aware 推薦中首個使用「任何閘」的方法，因 KG-aware 文獻浩繁，無法窮舉列舉。可辯護的主張為：\emph{在我們所檢視的方法中，沒有任何一個以偏置初始化的融合閘於稀疏或不可靠 KG 下提供架構優雅退化}。我們在第~\ref{ch:experiments-zh}~章的消融將 $4.5\%$ NDCG@20 的貢獻明確歸因於此閘的偏置初始化，支持上述主張在此情境中並非微不足道。
...
我們在第~\ref{ch:experiments-zh}~章報告一項單一資料集的觀察：對於我們的 aspect-saliency 合理化，將 softmax 替換為 sigmoid 會損失 $15.5\%$ NDCG@20，這是我們所量測到單一元件影響最大者。我們將其解讀為支持「正規化應與合理化的語意假設相符」此一設計啟發，但明確將此觀察定位為待多資料集複製驗證的假設（第~\ref{ch:conclusion-zh}~章）。
...
\begin{table}[ht]
\centering
\caption[從四個面向所作的設計比較。]{從四個面向所作的設計比較。NDCG@20 於我們的稀疏評論型 KG 基準上報告。}
...
\midrule
\textbf{RA-GARK} & \textbf{純 LightGCN} & \textbf{閘控後期融合} & \textbf{Aspect softmax} & \textbf{0.1242} \\
\bottomrule
\end{tabular}
\end{table}
