...
We probe RA-GARK's sensitivity along the four hyperparameter axes that a reader is most likely to question: the softmax temperature $\tau$, the number of latent aspect slots $A$, the fusion-gate bias $b$, and the contrastive weight $\lambda_{\mathrm{CL}}$. Each axis is swept with all other settings held at the values of Table~\ref{tab:hparams} and seed $= 42$. The best configuration, NDCG@$20 = 0.1242$, anchors the section. At the time of writing, the temperature and fusion-bias sweeps have completed, while the aspect-slot and contrastive-weight sweep rows in the experiment ledger are marked as reused cached outputs rather than fresh runs; we therefore list the settings now and leave the unverified values blank.

\subsection{Softmax Temperature $\tau$}
\label{sec:sens-tau}

Table~\ref{tab:sens-tau} reports NDCG@$20$ across $\tau \in \{0.05, 0.1, 0.5, 1.0\}$. The default $\tau = 0.5$ remains the best setting at $0.1242$. Sharpening to $\tau = 0.1$ or $\tau = 0.05$ lowers NDCG@$20$ to $0.1219$ and $0.1227$, respectively. The $\tau = 1.0$ row is listed for completeness, but its value is left blank until the fresh run is available. The picture is consistent with the analysis in Section~\ref{sec:softmax-softmax}: at $\tau = 0.5$ the slot logits are amplified enough to produce discriminative weights without collapsing onto a single slot. Pushing $\tau$ downward makes the rationale more selective and can discard useful mass on secondary slots, while pushing $\tau$ upward flattens the distribution toward a uniform item-wise mean over slots.
...
We planned to sweep $A \in \{2, 3, 4, 6, 8\}$, keeping the SVD-rank $k = A \cdot d$ scaled accordingly. The current experiment ledger, however, marks the non-default $A$ rows as reused cached outputs with identical metrics to the winner row, so these values are not valid sensitivity measurements. Table~\ref{tab:sens-A} therefore keeps the table structure and leaves the unverified entries blank. Until the fresh runs finish, we retain $A = 4$ as the default design value used in the main result, not as a measured optimum of this sweep.
...
The bias sweep is the most informative completed sensitivity run. Table~\ref{tab:sens-bias} shows NDCG@$20$ as $b$ is varied over $\{0, +2, +5, +10\}$. The default $b = +5$ is best. At $b = 0$ (no local bias) the model loses $3.7\%$ NDCG@$20$ ($0.1196$), and $b = +2$ recovers most of the gap ($0.1224$, $-1.4\%$). At $b = +10$ the gradient $\sigma'(b)$ becomes numerically small enough that the gate cannot adapt during training, and NDCG@$20$ falls to $0.1177$. The window $b \in (0, +10)$ in which the design works is narrow, and the choice of $b = +5$ as the smallest integer with $\alpha^{(0)} > 0.99$ and $\sigma'(b) > 10^{-3}$ (Section~\ref{sec:gate-bias-magnitude}) is empirically validated here.
...
We planned to sweep $\lambda_{\mathrm{CL}} \in \{0, 0.001, 0.005, 0.01, 0.05\}$. As with the aspect-slot sweep, the current experiment ledger marks the non-default contrastive-weight rows as reused cached outputs with identical metrics to the winner row. These entries are therefore left blank in Table~\ref{tab:sens-lambda}. The component ablations in Table~\ref{tab:ablation} still show that the two contrastive channels contribute smaller but non-trivial gains, but the shape of the full $\lambda_{\mathrm{CL}}$ sensitivity curve should be filled in only after fresh runs are available.
