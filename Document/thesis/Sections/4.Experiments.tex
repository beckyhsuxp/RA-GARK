...
\midrule
\textbf{RA-GARK (ours)}     & $\mathbf{0.1242}$ & $\mathbf{0.4972}$ & $\mathbf{0.2023}$ & $\mathbf{0.0592}$ \\
\midrule
$\Delta$ vs KGRec            & $+13.4\%$ & $+5.1\%$  & $+10.3\%$ & $+18.4\%$ \\
$\Delta$ vs LightGCN         & $+5.3\%$  & $+1.1\%$  & $+4.4\%$  & $+6.7\%$  \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
...
We measure each of the three headline design choices, KG-SVD initialization (Section~\ref{sec:kgsvd}), softmax rationale masking (Section~\ref{sec:softmax}), and the locally biased fusion gate (Section~\ref{sec:gate}), by individually reverting it and retraining from scratch with all other settings held fixed. We additionally include diagnostic rows that remove the global view, replace the per-(user, item) gate with a single learnable scalar $\alpha$, disable rationale selection, drop each contrastive channel individually, or restore the pre-fix sigmoid-plus-unbiased-gate configuration. Pure LightGCN is reproduced as the no-KG floor. All rows use seed $= 42$, and $\Delta$ denotes the relative NDCG@20 change against the full model.
...
\toprule
Configuration & NDCG@20 & NDCG@10 & $\Delta$ & What is removed \\
\midrule
\textbf{RA-GARK (full)}              & $\mathbf{0.1242}$ & $\mathbf{0.0960}$ & $0\%$       & none \\
\midrule
$-$ Softmax $\to$ sigmoid head       & $0.1003$ & $0.0785$ & $-19.2\%$ & cross-aspect competition (\S\ref{sec:softmax-vs-sigmoid}) \\
$-$ KG-SVD init $\to$ Xavier         & $0.1168$ & $0.0922$ & $-6.0\%$  & angular geometry at init (\S\ref{sec:kgsvd}) \\
$-$ Fusion-gate bias $b{=}5 \to 0$   & $0.1196$ & $0.0948$ & $-3.7\%$  & local-biased safe default (\S\ref{sec:gate-bias}) \\
$-$ MLP gate $\to$ scalar $\alpha$   & $0.1182$ & $0.0930$ & $-4.8\%$  & per-(u,i) gate conditioning (\S\ref{sec:gate-perside}) \\
$-$ User CL ($\mathcal{L}_{\mathrm{uCL}}$)  & $0.1191$ & $0.0923$ & $-4.1\%$  & user-side cross-view alignment \\
$-$ Aspect CL ($\mathcal{L}_{\mathrm{aCL}}$) & $0.1198$ & $0.0937$ & $-3.5\%$  & item-side aspect-level alignment \\
$-$ Rationale enabled $\to$ uniform mean & $0.1238$ & $0.0957$ & $-0.3\%$  & user-conditioned aspect selection \\
$-$ Global view (CL-only dual view)  & $0.1218$ & $0.0952$ & $-1.9\%$  & whole KG pipeline at score time \\
\midrule
LightGCN only (no-KG floor)          & $0.1179$ & $0.0908$ & ref.       & no global view; CF only \\
Pre-fix configuration (sigmoid + $b{=}0$) & $0.1051$ & $0.0827$ & $-15.4\%$ & both broken defaults reverted \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
