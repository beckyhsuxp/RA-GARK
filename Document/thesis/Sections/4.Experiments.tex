...
We measure each of the three headline design choices, KG-SVD initialization (Section~\ref{sec:kgsvd}), softmax rationale masking (Section~\ref{sec:softmax}), and the locally biased fusion gate (Section~\ref{sec:gate}), by individually reverting it and retraining from scratch with all other settings held fixed. We additionally include diagnostic rows that replace the per-(user, item) gate with a single learnable scalar $\alpha$, drop each contrastive channel individually, or restore the pre-fix sigmoid-plus-unbiased-gate configuration. Pure LightGCN is reproduced as the no-KG floor. All rows use seed $= 42$, and $\Delta$ denotes the relative NDCG@20 change against the full model.
...
\midrule
\textbf{RA-GARK (full)}              & $\mathbf{0.1242}$ & $\mathbf{0.0958}$ & $0\%$       & none \\
\midrule
$-$ Softmax $\to$ sigmoid head       & $0.1022$ & $0.0797$ & $-17.7\%$ & cross-aspect competition (\S\ref{sec:softmax-vs-sigmoid}) \\
$-$ KG-SVD init $\to$ Xavier         & $0.1171$ & $0.0923$ & $-5.7\%$  & angular geometry at init (\S\ref{sec:kgsvd}) \\
$-$ Fusion-gate bias $b{=}5 \to 0$   & $0.1175$ & $0.0915$ & $-5.4\%$  & local-biased safe default (\S\ref{sec:gate-bias}) \\
$-$ MLP gate $\to$ scalar $\alpha$   & $0.1184$ & $0.0929$ & $-4.7\%$  & per-(u,i) gate conditioning (\S\ref{sec:gate-perside}) \\
$-$ User CL ($\mathcal{L}_{\mathrm{uCL}}$)  & $0.1190$ & $0.0924$ & $-4.2\%$  & user-side cross-view alignment \\
$-$ Aspect CL ($\mathcal{L}_{\mathrm{aCL}}$) & $0.1200$ & $0.0938$ & $-3.4\%$  & item-side aspect-level alignment \\
\midrule
LightGCN only (no-KG floor)          & $0.1179$ & $0.0908$ & ref.       & no global view; CF only \\
Pre-fix configuration (sigmoid + $b{=}0$) & $0.1052$ & $0.0827$ & $-15.3\%$ & both broken defaults reverted \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
