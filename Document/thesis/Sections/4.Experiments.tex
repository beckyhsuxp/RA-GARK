...
\midrule
\textbf{RA-GARK (ours)}     & $\mathbf{0.1243}$ & $\mathbf{0.4972}$ & $\mathbf{0.2020}$ & $\mathbf{0.0594}$ \\
\midrule
...
We measure the contribution of each headline design choice--KG-SVD initialization (Section~\ref{sec:kgsvd}), softmax rationale masking (Section~\ref{sec:softmax}), and the locally biased fusion gate (Section~\ref{sec:gate})--by removing one component at a time and retraining from scratch with all other settings held fixed. We additionally include diagnostic rows that remove the global view, replace the per-(user, item) gate with a single learnable scalar $\alpha$, disable rationale selection, or drop each contrastive channel individually. All rows use seed $= 42$. Because the purpose of the ablation is to isolate ranking-quality changes, we report NDCG and MAP rather than coverage-oriented metrics such as HR and Recall.
...
\vspace{0.9em}
\noindent The Top-10 results show the same pattern.
...
Tables~\ref{tab:ablation@20} and~\ref{tab:ablation@10} support two observations.

\paragraph{Component contributions} Removing any one of the softmax rationale, KG-SVD initialization, local-biased fusion bias, or the per-(u,i) MLP gate lowers NDCG and MAP at both cutoffs. The largest degradation is the softmax head: NDCG@$20$ falls from $0.1243$ to $0.1005$, MAP@$20$ from $0.0594$ to $0.0451$, NDCG@$10$ from $0.0960$ to $0.0785$, and MAP@$10$ from $0.0519$ to $0.0397$. The other three components produce smaller but consistent rank-quality losses. KG-SVD removal gives NDCG@$20 = 0.1171$ and MAP@$20 = 0.0545$; removing the fusion-gate bias gives $0.1194$ and $0.0555$; replacing the per-(u,i) MLP gate gives $0.1180$ and $0.0552$.
...
The figure is intended as a trend view rather than a second results table: it shows whether each design choice is locally stable around the default setting.
...
Figure~\ref{fig:case-heatmap} makes two empirical patterns clear. Neither matches the strong per-(user, item) selection story one might naively expect from a rationale module.
