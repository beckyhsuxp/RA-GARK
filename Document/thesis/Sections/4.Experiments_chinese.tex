...
表~\ref{tab:main-zh} 報告主要測試集結果。RA-GARK 取得 NDCG@$20 = 0.1243$，較最強 KG-aware baseline（KGRec）提升 $+13.5\%$，較純 LightGCN 提升 $+5.4\%$。提升於各次要排序指標（HR@$20$、Recall@$20$、MAP@$20$）皆一致；對 KGRec 之最大相對增益為 MAP@$20$ 的 $+18.8\%$，顯示正確檢索物品之排序品質亦優於 baseline。作為更嚴格 cutoff 之檢查，RA-GARK 在 Top-10 亦優於純 LightGCN，NDCG@$10 = 0.0960$，對照純 LightGCN 之 $0.0908$。
...
\midrule
\textbf{RA-GARK（本文）}     & $\mathbf{0.1243}$ & $\mathbf{0.4972}$ & $\mathbf{0.2020}$ & $\mathbf{0.0594}$ \\
\midrule
...
此表有兩種讀法值得分開敘述。其一（較 KGRec $+13.5\%$）顯示在相同 KG 下，RA-GARK 的設計選擇在同一資料集上產生比最強既有 KG-aware 方法更佳的推薦器。其二（較純 LightGCN $+5.4\%$）在方法論上更具意義。在同一稀疏 KG 情境中，四個 KG-aware baseline 皆低於無 KG 對照；RA-GARK 是此處所評之五個 KG-aware 配置中，唯一將 KG 訊號由淨負債轉為淨資產者。促成此翻轉之架構機制，也就是將 KG 通道置於偏置初始化之融合閘後，使其僅在貢獻降低損失時才啟用，將於下一節詳細審視。
...
我們對三項核心設計選擇，KG-SVD 初始化（第~\ref{sec:kgsvd-zh}~節）、Softmax 合理化遮罩（第~\ref{sec:softmax-zh}~節）、區域偏置融合閘（第~\ref{sec:gate-zh}~節），逐一移除並於其他設定固定下從頭重新訓練，以衡量其貢獻。另外加入數個診斷列：完全移除全域視角、以單一可學習純量 $\alpha$ 取代 per-(使用者, 物品) 閘、停用合理化選擇，或分別丟棄兩條對比通道。所有列皆使用種子 $= 42$。由於消融分析的目的在於隔離排序品質變化，本文回報 NDCG 與 MAP，而不列入 HR 與 Recall 等偏向覆蓋率的指標。
...
此圖作為趨勢檢視，而非第二份結果表；其重點在於各設計選擇於預設值附近是否穩定。
...
圖~\ref{fig:case-heatmap-zh} 呈現兩項明確之經驗模式，二者皆與一般對合理化模組所期望的強 per-(使用者, 物品) 選擇敘事不符。
