...
我們衡量主要架構選擇之貢獻，方式為逐一移除單一元件並在其他設定固定下從頭重新訓練。為了讓結構更清楚，這些消融分為三類：核心架構元件（KG-SVD 初始化、Softmax 合理化遮罩、區域偏置融合閘）、輔助正則項（使用者側與面向側對比損失），以及診斷變體（移除全域視角、以單一可學習純量 $\alpha$ 取代 per-(使用者, 物品) 閘、或停用合理化選擇）。所有列皆使用種子 $= 42$。由於消融分析的目的在於隔離排序品質變化，本文回報 NDCG 與 MAP，而不列入 HR 與 Recall 等偏向覆蓋率的指標；表~\ref{tab:ablation@20} 與表~\ref{tab:ablation@10} 分別呈現相同消融集合於 Top-20 與 Top-10 下的結果。
...
NDCG@$20$ 反映較大幅度的整體排序退化；NDCG@$10$ 則使用更嚴格 cutoff，因此絕對降幅略小。兩者的相對排序仍然穩定：核心架構列造成最大變化，對比列提供較小但一致的輔助效果，而診斷變體則確認這些增益不是單一 toggle 的偶然產物。讀這兩張表時，重點應放在同一排名模式是否跨 cutoff 持續成立，而不是某一個 delta 的絕對大小。
...
\paragraph{元件貢獻} 綜合兩個消融 cutoff，核心架構列主導排序損失，而對比列則提供較小但一致的輔助效果。影響最大者為 softmax 頭：NDCG@$20$ 由 $0.1243$ 降至 $0.1005$，MAP@$20$ 由 $0.0594$ 降至 $0.0451$，NDCG@$10$ 由 $0.0960$ 降至 $0.0785$，MAP@$10$ 由 $0.0519$ 降至 $0.0397$。其他核心元件亦不可忽略：移除 KG-SVD 時 NDCG@$20 = 0.1171$、MAP@$20 = 0.0545$；移除融合閘偏置時為 $0.1194$ 與 $0.0555$；以純量閘取代 per-(u,i) MLP 閘時為 $0.1180$ 與 $0.0552$。兩條對比損失的幅度較小，但維持穩定收益。
