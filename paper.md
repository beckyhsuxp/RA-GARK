...
![Figure 1. RA-GARK 整體架構（橫版）。兩條平行的 swim lane 分別是 Local View（LightGCN，協同訊號）與 Global View（KG-aspect，語意訊號）；兩視角各自送入 user / item fusion gate 合併，產出 u_final 與 i_final 的內積作為分數。三顆 ★ 分別標示本文的三項創新：KG SVD 初始化、Softmax + τ rationale masking、以及 bias = +5 的 local-biased fusion gate 初始化。虛線表示 stop-grad 的 CL 路徑。](figures/architecture.png)
